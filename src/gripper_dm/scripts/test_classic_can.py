#!/usr/bin/env python3
"""
测试：使用经典 CAN（而不是 CAN-FD）发送 MIT 控制
"""
...
def main():
    ...
    try:
        ...
        print("\n[2] 使用经典 CAN 发送 MIT 控制 (目标: 0.5 rad)...")
        PMAX = 12.566
        VMAX = 30.0
        TMAX = 10.0
        ...
        for _ in range(100):
            driver.transmit(CAN_ID, bytes(mit_data))  # 使用经典 CAN
            time.sleep(0.01)
        
        time.sleep(0.5)
        # 尝试接收经典 CAN 和 CAN-FD
        frames = driver.receive(count=20, timeout_ms=200)
        frames_fd = driver.receive_fd(count=20, timeout_ms=200)
        all_frames = frames + frames_fd
        print(f"  收到 {len(all_frames)} 帧 (经典: {len(frames)}, CAN-FD: {len(frames_fd)})")
        if all_frames:
            pos = parse_feedback(all_frames[-1]['data'])
            if pos is not None:
                print(f"  反馈位置: {pos:.4f} rad")
                print(f"  移动: {abs(pos - initial_pos) > 0.05}")

        print("\n[3] 使用经典 CAN 发送 MIT 控制 (目标: 1.0 rad)...")
        q_des = 1.0
        q_uint = int((q_des + PMAX) / (2 * PMAX) * 65535)
        mit_data[0] = (q_uint >> 8) & 0xFF
        mit_data[1] = q_uint & 0xFF
        ...
        for _ in range(100):
            driver.transmit(CAN_ID, bytes(mit_data))
            time.sleep(0.01)
